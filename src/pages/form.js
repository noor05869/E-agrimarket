...
function Form() {
  ...
  async function handleSubmit() {
    ...
    try {
      console.log(initialValues1.cropsDiss,"res =======================================", initialValues1.cropsDiss);
...
let ctdata=cattless.filter((element ,index) => {
   ...
   if(!Object.values(element).includes('')){
     return element
      ...
      const { data } = await axios.post(
        "https://backend.eagrimarket.com/v1/personalinof",
        {
          personalInfo: {
 ...
 const disabledEndDate = (i) => {
   console.log("disssssssssssssss",cropDistribution[i].endDate,i,"startttttttttttt",cropDistribution[i]?.startDate)
    // const startValue = cropDistribution[i]?.startDate;
